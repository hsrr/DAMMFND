...
class Run():
    def __init__(self,
        ...
        if config['dataset'] == "weibo":
            ...
            self.train_path = self.root_path + 'train_2_domain.xlsx'
            self.val_path = self.root_path + 'val_2_domain.xlsx'
            self.test_path = self.root_path + 'test_2_domain.xlsx'
            self.category_dict = {
                "科技": 0, "军事": 1, "教育考试": 2, "灾难事故": 3, "政治": 4,
                "医药健康": 5, "财经商业": 6, "文体娱乐": 7, "社会生活": 8
            }
        elif config['dataset'] == "sixclass":
            self.data_root = config.get('data_root', '/map-vepfs/liniuniu/hesirui/datasets')
            self.image_root = config.get('image_root', self.data_root)
            self.train_path = os.path.join(self.data_root, 'train.jsonl')
            self.val_path = os.path.join(self.data_root, 'val.jsonl')
            self.test_path = os.path.join(self.data_root, 'test.jsonl')
            self.category_dict = {
                "real_news": 0, "image_forgery": 1, "entity_inconsist": 2,
                "event_inconsist": 3, "time_inconsist": 4, "invalid_visual": 5
            }
            self.num_classes = 6

    def get_dataloader(self, dataset):
        if dataset == "sixclass":
            loader = sixclass_data(max_len=self.max_len, batch_size=self.batchsize,
                                   vocab_file=self.vocab_file, num_workers=self.num_workers)
            train_loader = loader.load_data(self.train_path, self.image_root, True)
            val_loader = loader.load_data(self.val_path, self.image_root, False)
            test_loader = loader.load_data(self.test_path, self.image_root, False)
